...
class JokeCreateView(SuccessMessageMixin, LoginRequiredMixin, CreateView):
    model = Joke
    form_class = JokeForm
    success_message = 'Success Joke Created'
...
def vote(request, slug):
    ...
    user_vote = data['vote'] # The user's new vote.
    ...
    if user.is_anonymous: # User not logged in. Can't vote.
        ...
        if JokeVote.objects.filter(user=user, joke=joke).exists():
            ...
            if joke_vote.vote == user_vote: # User's new vote is the same as old vote.
                msg = 'Right. You told us already. Geez.'
            else: # User changed vote.
                joke_vote.vote = user_vote # Update JokeVote instance.
                joke_vote.save() # Save.
            ...
            joke_vote = JokeVote(user=user, joke=joke, vote=user_vote)
            joke_vote.save()
